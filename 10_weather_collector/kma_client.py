...
def parse_weather(raw: dict, site_id: str) -> dict:
    ...
    return {
        ...
        "wind_max":        max_wind,
        "max_ins_wind":    max_ins_wind,                      # 순간최대풍속 (m/s) ★추가
        "snow_depth":      snow_depth,
        "humidity_avg":    float(raw.get("avgRhm")  or 0),   # 평균습도 (%)
        "sunshine_hours":  sunshine_hours,                    # 일조시간 (hr) ★추가

        # ── 작업불가일 판정 플래그 ──────────────────────
        "is_rain_day":     precipitation  >= 10,   # 강수 10mm 이상
        "is_wind_day":     max_wind       >= 14,   # 최대풍속 14m/s 이상
        "is_wind_crane":   max_ins_wind   >= 10,   # 크레인 작업 제한 (순간 10m/s) ★추가
        "is_snow_day":     snow_depth     >= 1,    # 적설 1cm 이상
        "is_heat_day":     temp_max       >= 35,   # 폭염 35℃ 이상
        "is_cold_day":     temp_min       <= -10,  # 한파 -10℃ 이하
        "is_no_sunshine":  sunshine_hours < 2,     # 일조 2시간 미만 ★추가
        "rain_yn":         raw.get("rnDay") == "1",  # 강수 유무 (소량 포함) ★추가
        "fog_yn":          raw.get("fogDay") == "1", # 안개 유무 ★추가
    }