...
def collect(folder: Path, index_stack: list, tmp_dir: str,
    ...
    start_number: 최상위 레벨에서만 적용되는 시작 번호 오프셋.
                  index_stack이 비어있으면(최상위) idx가 start_number-1부터 시작.
    """
    # 최상위 레벨이면 start_number 적용, 하위 레벨은 항상 0부터
    ...
    for entry in sorted_entries(folder):
        ...
        if entry.is_dir():
            ...
            stats['covers'] += 1
            # 하위 레벨은 start_number 영향 없음 (기본 1)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PDF 합치기 도구 v3")
        self.geometry("720x700")
        self.resizable(True, True)
        self.cover_entries = []  # 초기 갑지 입력 위젯 목록
        self._build_ui()
        # 기본값으로 2개 갑지 추가
    ...
    def _build_ui(self):
        ...
        self.cover_scrollbar.pack_forget()  # 스크롤바는 필요시만
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        # CLI 모드 (--covers "갑지1,갑지2" --start 3)
        import argparse
        parser = argparse.ArgumentParser(description="PDF 합치기 도구")
