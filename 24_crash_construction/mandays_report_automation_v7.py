# -*- coding: utf-8 -*-
"""
돌관공사비 산출근거 - 노무비 출력일보 자동 작성 스크립트 (v7)

[v7 주요 변경 사항]
1) 페이지 분할 (한 페이지 최대 21명)
   - 섹션당 근로자가 22명 이상이면 여러 페이지로 분할
   - 각 페이지마다 표 제목(헤더 6행) 재출력
   - 중간 페이지: 헤더 + 근로자 21명 (합계행 없음)
   - 마지막 페이지: 헤더 + 남은 근로자 + 빈 행으로 21명 채움 + 합계행
   - 합계행은 섹션 전체 (모든 페이지의 근로자) 기준으로 계산
2) AB열 (노무비 단가) - 소스 파일 B열 "일 기준" 아래 값을 근로자별로 적용
3) AC열 (노무비 총액) - 소스 파일 E열 근로자 블록 마지막 총액 값 적용

[v6 주요 변경 사항]
- External link + definedNames 제거 (Excel 복구 경고 방지)
- ArrayFormula 재생성 (V~Y 수식이 정상 계산되도록)
- 섹션별 헤더 참조 올바르게 이동 (absolute_delta)
...
2. 터미널에서:
       ...
       python mandays_report_automation_v7.py
"""
...
OUTPUT_PATH   = os.environ.get('CRASH_OUTPUT_PATH',
    os.path.join(_BASE, '돌관공사비_산출근거_자동생성_v7.xlsx'))
...
COL_UNIT_PRICE = 28  # AB: 노무비 단가 (v7)
COL_TOTAL = 29       # AC: 노무비 총액 (v7)
