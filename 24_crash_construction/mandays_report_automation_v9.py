# -*- coding: utf-8 -*-
"""
돌관공사비 산출근거 - 노무비 출력일보 자동 작성 스크립트 (v9)

[v9 주요 변경 사항]
- AC열(노무비 총액) 계산 방식 변경:
  기존(v7~v8): 소스 E열 '전체' 합계를 모든 카테고리에 동일하게 표시
  변경(v9): 해당 근로자가 그 카테고리(본선/복합/삼성/기타)에 투입된
            날짜의 E열 값만 합산 → 카테고리별로 다른 총액
...
2. 터미널에서:
       ...
       python mandays_report_automation_v9.py
"""
...
OUTPUT_PATH   = os.environ.get('CRASH_OUTPUT_PATH',
    os.path.join(_BASE, 'output', '돌관공사비_산출근거_자동생성_v9.xlsx'))
...
COL_UNIT_PRICE = 28  # AB: 노무비 단가 (v9)
COL_TOTAL = 29       # AC: 노무비 총액 (v9)
