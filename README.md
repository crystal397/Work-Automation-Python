...
- 슬롯 크기의 흰 캔버스에 사진을 중앙 배치 → 앵커 오차 무관하게 항상 정확한 중앙 정렬
...
- **분류기 회귀 테스트**: 다양한 발신자 형식 (도급사·감리단·발주자·관계기관·타 시공사) 통과
...
**검증**: 익명 케이스 시연 — 자동 검증 통과 다수, PNG 출처 자동 캡쳐, 한글↔숫자 불일치 자동 검출.
...
- **하도급 산재 0% 분기**: 보고서 규칙 자동 적용 (`company_role == "sub"` 분기)
- **어댑터 5종**: 인원투입현황 xlsx, 월별급여현황표 xlsx(일할 산정), 지불조서 PDF, 인원별 명세서 PDF, 보고서 본문 4.3.2.2 표 cross_check
...
**검증**: 익명 케이스 시연 — 의뢰인 보고서 산정값 100% 일치, 일할 산정 검증 완료.
...
- **다개월 일괄**: 파일명 YY.MM 매칭으로 폴더 단위 처리
...
**검증**: 익명 케이스 시연 — 장기 xlsx 일괄 처리, 월별 매트릭스 자동 생성.

**미구현**: 24의 정교한 출력일보(인쇄영역·머지셀·48행·휴일), 12 회사별 어댑터, 산출내역서 노임 시트 자동 기입.
...
| 33 귀책분석 신 파이프라인 | 다양한 발신자 양식 회귀 통과, 5종 자동 검증, 어조 자동 변환 | pydantic v2, /analyze-claim 글로벌 슬래시, bridge 머지 |
| 36 계약 메타 | 익명 케이스 검증 통과, 한글↔숫자 불일치 자동 검출 | Sourced[T] 출처 의무, 본문 docx, 5장 합철 |
| 37 간접비 산정 | 익명 케이스 의뢰인 보고서 산정값 100% 일치 | 산식 자동, 하도급 산재 0% 분기, 일할 산정 |
| 38 돌관공사비 신 | 장기 xlsx 일괄 매트릭스 생성, 24의 발전형 | 24 v12 패턴, 매트릭스 xlsx, 다개월 일괄 |
